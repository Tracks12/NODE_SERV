...
function editor() {
	...
	), syntax = new Array( 'var', 'if', 'else', 'switch', 'case', 'default', 'break', 'for', 'while', 'function', 'try', 'catch', '\''), s;
			...
			for(var k = 0; k < syntax.length; k++) {
				...
				if(pre[i].getElementsByTagName('li')[j].innerHTML.split(syntax[k])[1]) {
					switch(k) {
						...
						case 11: pre[i].getElementsByTagName('li')[j].innerHTML = sep[0]+'<font color="#3333FF">'+syntax[k]+'</font>'+sep[1]; break;
						case 12: pre[i].getElementsByTagName('li')[j].innerHTML = '';
